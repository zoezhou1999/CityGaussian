...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder, masks_folder):
    ...
    if os.path.exists(masks_folder):
        print("Masks folder detected, reading masks")
    for idx, key in enumerate(cam_extrinsics):
        ...
        if image_name.startswith(('top', 'left', 'right', 'front', 'back')):
            image_path = os.path.join(images_folder, image_name.split("_")[0], os.path.basename(extr.name))
        ...
        if os.path.exists(masks_folder):
            ...
            if mask_name.startswith(('top', 'left', 'right', 'front', 'back')):
                mask_path = os.path.join(masks_folder, mask_name.split("_")[0], mask_name + '.png')
...
def readColmapSceneInfo(path, images, eval, llffhold=50, partition=None):
    try:
